...
export function AnthropicSettingsView() {
    ...
    const [editingKey, setEditingKey] = useState<AnthropicKey | null>(null);
    const [keyForm, setKeyForm] = useState({ name: '', apiKey: '', showApiKey: false });
    const [isValidatingKey, setIsValidatingKey] = useState(false);
    const [isKeyModalOpen, setIsKeyModalOpen] = useState(false);

    // Models management state
    const [modelConfigs, setModelConfigs] = useState<AnthropicModelConfig[]>([]);
    ...
    // Fetch Anthropic keys
    const { data: keysData, isLoading: isLoadingKeys, error: keysError } = useQuery({
        queryKey: ['anthropic-keys'],
        queryFn: async () => {
            const response = await apiClient.request<{ success: boolean; data: AnthropicKey[] }>('/anthropic-providers');
            if (!response.success) {
                throw new Error(response.message || 'Failed to fetch Anthropic providers');
            }
    ...
    const createKeyMutation = useMutation({
        mutationFn: async (keyData: { name: string; apiKey: string; isActive: boolean }) => {
            ...
            if (!response.success) {
                throw new Error(response.message || 'Failed to create Anthropic provider');
            }
            return response.data;
        },
        onSuccess: (newKey) => {
            queryClient.invalidateQueries({ queryKey: ['anthropic-keys'] });
            addToast({
                title: "Success",
                description: "Anthropic API key added successfully",
                variant: "default"
            });
            handleCloseKeyModal();
        },
        onError: (error) => {
            addToast({
                title: "Error",
                description: error instanceof Error ? error.message : "Failed to add Anthropic API key",
                variant: "destructive"
            });
        }
    });
    ...
    const updateKeyMutation = useMutation({
        mutationFn: async ({ id, keyData }: { id: string; keyData: { name?: string; apiKey?: string; isActive?: boolean } }) => {
            const response = await apiClient.request<{ success: boolean; data: AnthropicKey }>(`/anthropic-providers/${id}`, {
                method: 'PUT',
                body: JSON.stringify(keyData),
            });
            if (!response.success) {
                throw new Error(response.message || 'Failed to update Anthropic provider');
            }
            return response.data;
        },
        onSuccess: () => {
        ...
        onError: (error) => {
            addToast({
                title: "Error",
                description: error instanceof Error ? error.message : "Failed to update Anthropic API key",
                variant: "destructive"
            });
        }
    });
    ...
    const deleteKeyMutation = useMutation({
        mutationFn: async (id: string) => {
            const response = await apiClient.request<{ success: boolean }>(`/anthropic-providers/${id}`, {
                method: 'DELETE',
            });
            if (!response.success) {
                throw new Error(response.message || 'Failed to delete Anthropic provider');
            }
        },
        onSuccess: () => {
            queryClient.invalidateQueries({ queryKey: ['anthropic-keys'] });
            addToast({
                title: "Success",
                description: "Anthropic API key deleted successfully",
                variant: "default"
            });
        },
        onError: (error) => {
            addToast({
                title: "Error",
                description: error instanceof Error ? error.message : "Failed to delete Anthropic API key",
                variant: "destructive"
            });
        }
    });
    ...
    const testKeyMutation = useMutation({
        mutationFn: async (id: string) => {
            const response = await apiClient.request<{ success: boolean }>(`/anthropic-providers/${id}/test`, {
                method: 'POST',
            });
            if (!response.success) {
                throw new Error(response.message || 'Failed to test Anthropic API key');
            }
        },
        onSuccess: () => {
            addToast({
        ...
        onError: (error) => {
            addToast({
                title: "Error",
                description: error instanceof Error ? error.message : "Failed to test Anthropic API key",
                variant: "destructive"
            });
        }
    });
    ...
    const handleEditKey = (key: AnthropicKey) => {
        setEditingKey(key);
    ...
    const fetchModelsForKey = async (keyId: string) => {
        ...
        try {
            ...
            const keyResponse = await apiClient.request<{ success: boolean; data: { apiKey: string } }>(`/anthropic-providers/${keyId}/decrypted-key`);
            if (!keyResponse.success) {
                throw new Error(keyResponse.message || 'Failed to get API key');
            }

            const result = await apiClient.fetchAnthropicModels({ apiKey: keyResponse.data.apiKey });
    ...
    const handleAddModel = async (modelId: string, customName?: string) => {
        // For now, we'll just show a success message since we don't have model configs for Anthropic yet
        addToast({
            title: "Success",
            ...
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                        Manage your Anthropic API keys for Claude models
                    </p>
                </div>
                ...
                <Card>
                    <CardContent className="flex flex-col items-center justify-center py-8">
                        ...
                        <p className="text-sm text-muted-foreground text-center mb-4">
                            Add your first Anthropic API key to get started with Claude models
                        </p>
                        <Button onClick={handleAddKey} className="flex items-center gap-2">
                            <Plus className="h-4 w-4" />
                            Add API Key
        ...
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                        Configure Claude models for your API keys
                    </p>
                </div>
                {keys.length > 0 && (
                    <Button
            ...
            {keys.length === 0 ? (
                <Card>
                    <CardContent className="flex flex-col items-center justify-center py-8">
                        ...
                        <h3 className="text-lg font-medium mb-2">No API Keys</h3>
                        <p className="text-sm text-muted-foreground text-center mb-4">
                            Add an Anthropic API key first to configure Claude models
                        </p>
            ...
            <Dialog open={isKeyModalOpen} onOpenChange={setIsKeyModalOpen}>
                <DialogContent className="sm:max-w-[425px]">
                    <DialogHeader>
                        ...
                        <DialogDescription>
                            {editingKey
                                ? 'Update your Anthropic API key configuration'
                                : 'Add a new Anthropic API key with a custom name'
                            }
                        </DialogDescription>
                    </DialogHeader>
                        ...
                        <div className="grid gap-2">
                            ...
                            <div className="relative">
                                <Input
                                    ...
                                    placeholder={editingKey ? "Enter new API key (optional)" : "sk-ant-..."}
                                />
                                <Button
                                    type="button"
                                    variant="ghost"
