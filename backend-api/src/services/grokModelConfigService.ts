...
import {
    ...
    GrokModelConfigResponse,
    ModelCapabilities,
    ModelPricing
} from '../types/ai-provider';

export class GrokModelConfigService {
    private static instance: GrokModelConfigService;
    ...
    public static getInstance(): GrokModelConfigService {
        if (!GrokModelConfigService.instance) {
            GrokModelConfigService.instance = new GrokModelConfigService();
        }
        return GrokModelConfigService.instance;
    }
    ...
    // Get key information from grok_providers table
            ...
            switch (dbType) {
                case 'localdb':
                    return new Promise<{ name: string; apiKey: string } | null>((resolve, reject) => {
                        connection.get(
                            'SELECT name, apiKey FROM grok_providers WHERE id = ? AND isActive = 1',
                            [keyId],
                            (err: any, row: any) => {
                                if (err) reject(err);
                ...
                case 'mongodb':
                    const provider = await connection.collection('grok_providers').findOne({ id: keyId, isActive: true });
                    return provider ? { name: provider.name, apiKey: this.decryptApiKey(provider.apiKey) } : null;

                case 'mysql':
                case 'postgresql':
                    const [rows] = await connection.execute(
                        'SELECT name, apiKey FROM grok_providers WHERE id = ? AND isActive = TRUE',
                        [keyId]
                    );
    ...
    public async createModelConfig(request: CreateGrokModelConfigRequest): Promise<GrokModelConfigResponse> {
        try {
    ...
    public async getAllModelConfigs(): Promise<GrokModelConfigResponse> {
        try {
            const config = this.dbManager.getCurrentConfig();
            if (!config) {
                throw new Error('No database configuration found');
    ...
    public async updateModelConfig(id: string, request: UpdateGrokModelConfigRequest): Promise<GrokModelConfigResponse> {
        try {
            const config = this.dbManager.getCurrentConfig();
            if (!config) {
                throw new Error('No database configuration found');
            ...
            const updatedConfig: GrokModelConfig = {
                ...existingConfig,
    ...
    public async deleteModelConfig(id: string): Promise<GrokModelConfigResponse> {
        try {
            const config = this.dbManager.getCurrentConfig();
            if (!config) {
                throw new Error('No database configuration found');
    ...
    private async getModelConfigByKeyAndModel(connection: any, dbType: string, keyId: string, modelId: string): Promise<GrokModelConfig | null> {
        try {
            switch (dbType) {
                case 'localdb':
                    return new Promise<GrokModelConfig | null>((resolve, reject) => {
                        connection.get(
                            'SELECT * FROM grok_model_configs WHERE keyId = ? AND modelId = ?',
                            [keyId, modelId],
                            (err: any, row: any) => {
                                if (err) reject(err);
                ...
                case 'mongodb':
                    const modelConfig = await connection.collection('grok_model_configs').findOne({ keyId, modelId });
    ...
    private async getModelConfigById(connection: any, dbType: string, id: string): Promise<GrokModelConfig | null> {
        try {
            switch (dbType) {
                case 'localdb':
                    return new Promise<GrokModelConfig | null>((resolve, reject) => {
                        connection.get(
                            'SELECT * FROM grok_model_configs WHERE id = ?',
                            [id],
                            (err: any, row: any) => {
                                if (err) reject(err);
                                else resolve(row ? this.parseModelConfigFromDB(row, dbType) : null);
                ...
                case 'mongodb':
                    const modelConfig = await connection.collection('grok_model_configs').findOne({ id });
                    return modelConfig ? this.parseModelConfigFromDB(modelConfig, dbType) : null;
    ...
    private async getModelConfigsFromDatabase(connection: any, dbType: string): Promise<GrokModelConfig[]> {
        switch (dbType) {
            case 'localdb':
                return new Promise<GrokModelConfig[]>((resolve, reject) => {
                    connection.all(
                        'SELECT * FROM grok_model_configs ORDER BY createdAt DESC',
                        [],
                        (err: any, rows: any[]) => {
                            if (err) reject(err);
            ...
            case 'mongodb':
                const configs = await connection.collection('grok_model_configs').find({}).sort({ createdAt: -1 }).toArray();
    ...
    private async saveModelConfigToDatabase(connection: any, dbType: string, modelConfig: GrokModelConfig): Promise<void> {
        ...
        switch (dbType) {
            case 'localdb':
                return new Promise<void>((resolve, reject) => {
                    connection.run(
                        `INSERT INTO grok_model_configs
            ...
            case 'mongodb':
                await connection.collection('grok_model_configs').insertOne({
                    ...modelConfig,
    ...
    private async updateModelConfigInDatabase(connection: any, dbType: string, id: string, modelConfig: GrokModelConfig): Promise<void> {
        const capabilitiesStr = JSON.stringify(modelConfig.capabilities || {});
        const pricingStr = JSON.stringify(modelConfig.pricing || {});
        ...
        switch (dbType) {
            ...
            case 'mongodb':
                await connection.collection('grok_model_configs').updateOne(
                    { id },
                    {
                        $set: {
    ...
    private async deleteModelConfigFromDatabase(connection: any, dbType: string, id: string): Promise<void> {
        switch (dbType) {
            case 'localdb':
                return new Promise<void>((resolve, reject) => {
                    connection.run('DELETE FROM grok_model_configs WHERE id = ?', [id], (err: any) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
            ...
            case 'mongodb':
                await connection.collection('grok_model_configs').deleteOne({ id });
                break;

            case 'mysql':
            case 'postgresql':
                await connection.execute('DELETE FROM grok_model_configs WHERE id = ?', [id]);
                break;
    ...
    private parseModelConfigFromDB(row: any, dbType: string): GrokModelConfig {
