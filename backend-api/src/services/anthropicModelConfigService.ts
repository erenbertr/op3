...
import {
    ...
    AnthropicModelConfigResponse,
    ModelCapabilities,
    ModelPricing
} from '../types/ai-provider';

export class AnthropicModelConfigService {
    private static instance: AnthropicModelConfigService;
    ...
    public static getInstance(): AnthropicModelConfigService {
        if (!AnthropicModelConfigService.instance) {
            AnthropicModelConfigService.instance = new AnthropicModelConfigService();
        }
        return AnthropicModelConfigService.instance;
    }
        ...
        switch (dbType) {
            case 'localdb':
                return new Promise<void>((resolve, reject) => {
                    connection.run(`
                        CREATE TABLE IF NOT EXISTS anthropic_model_configs (
                            id TEXT PRIMARY KEY,
                            keyId TEXT NOT NULL,
    ...
    // Get key information from anthropic_providers table
            ...
            switch (dbType) {
                case 'localdb':
                    return new Promise<{ name: string; apiKey: string } | null>((resolve, reject) => {
                        connection.get(
                            'SELECT name, apiKey FROM anthropic_providers WHERE id = ? AND isActive = 1',
                            [keyId],
                            (err: any, row: any) => {
                                if (err) reject(err);
                ...
                case 'mongodb':
                    const provider = await connection.collection('anthropic_providers').findOne({ id: keyId, isActive: true });
                    return provider ? { name: provider.name, apiKey: this.decryptApiKey(provider.apiKey) } : null;

                case 'mysql':
                case 'postgresql':
                    const [rows] = await connection.execute(
                        'SELECT name, apiKey FROM anthropic_providers WHERE id = ? AND isActive = TRUE',
                        [keyId]
                    );
    ...
    public async createModelConfig(request: CreateAnthropicModelConfigRequest): Promise<AnthropicModelConfigResponse> {
        try {
    ...
    public async getAllModelConfigs(): Promise<AnthropicModelConfigResponse> {
        try {
            const config = this.dbManager.getCurrentConfig();
            if (!config) {
                throw new Error('No database configuration found');
    ...
    public async updateModelConfig(id: string, request: UpdateAnthropicModelConfigRequest): Promise<AnthropicModelConfigResponse> {
        try {
            const config = this.dbManager.getCurrentConfig();
            if (!config) {
                throw new Error('No database configuration found');
            ...
            const updatedConfig: AnthropicModelConfig = {
                ...existingConfig,
    ...
    public async deleteModelConfig(id: string): Promise<AnthropicModelConfigResponse> {
        try {
            const config = this.dbManager.getCurrentConfig();
            if (!config) {
                throw new Error('No database configuration found');
    ...
    private async getModelConfigByKeyAndModel(connection: any, dbType: string, keyId: string, modelId: string): Promise<AnthropicModelConfig | null> {
        try {
            switch (dbType) {
                case 'localdb':
                    return new Promise<AnthropicModelConfig | null>((resolve, reject) => {
                        connection.get(
                            'SELECT * FROM anthropic_model_configs WHERE keyId = ? AND modelId = ?',
                            [keyId, modelId],
                            (err: any, row: any) => {
                                if (err) reject(err);
                ...
                case 'mongodb':
                    const modelConfig = await connection.collection('anthropic_model_configs').findOne({ keyId, modelId });
    ...
    private async getModelConfigById(connection: any, dbType: string, id: string): Promise<AnthropicModelConfig | null> {
        try {
            switch (dbType) {
                case 'localdb':
                    return new Promise<AnthropicModelConfig | null>((resolve, reject) => {
                        connection.get(
                            'SELECT * FROM anthropic_model_configs WHERE id = ?',
                            [id],
                            (err: any, row: any) => {
                                if (err) reject(err);
                                else resolve(row ? this.parseModelConfigFromDB(row, dbType) : null);
                ...
                case 'mongodb':
                    const modelConfig = await connection.collection('anthropic_model_configs').findOne({ id });
                    return modelConfig ? this.parseModelConfigFromDB(modelConfig, dbType) : null;
    ...
    private async getModelConfigsFromDatabase(connection: any, dbType: string): Promise<AnthropicModelConfig[]> {
        switch (dbType) {
            case 'localdb':
                return new Promise<AnthropicModelConfig[]>((resolve, reject) => {
                    connection.all(
                        'SELECT * FROM anthropic_model_configs ORDER BY createdAt DESC',
                        [],
                        (err: any, rows: any[]) => {
                            if (err) reject(err);
            ...
            case 'mongodb':
                const configs = await connection.collection('anthropic_model_configs').find({}).sort({ createdAt: -1 }).toArray();
    ...
    private async saveModelConfigToDatabase(connection: any, dbType: string, modelConfig: AnthropicModelConfig): Promise<void> {
        ...
        switch (dbType) {
            case 'localdb':
                return new Promise<void>((resolve, reject) => {
                    connection.run(
                        `INSERT INTO anthropic_model_configs
            ...
            case 'mongodb':
                await connection.collection('anthropic_model_configs').insertOne({
                    ...modelConfig,
    ...
    private async updateModelConfigInDatabase(connection: any, dbType: string, id: string, modelConfig: AnthropicModelConfig): Promise<void> {
        const capabilitiesStr = JSON.stringify(modelConfig.capabilities || {});
        const pricingStr = JSON.stringify(modelConfig.pricing || {});
        ...
        switch (dbType) {
            case 'localdb':
                return new Promise<void>((resolve, reject) => {
                    connection.run(
                        `UPDATE anthropic_model_configs SET
            ...
            case 'mongodb':
                await connection.collection('anthropic_model_configs').updateOne(
                    { id },
                    {
                        $set: {
    ...
    private async deleteModelConfigFromDatabase(connection: any, dbType: string, id: string): Promise<void> {
        switch (dbType) {
            case 'localdb':
                return new Promise<void>((resolve, reject) => {
                    connection.run('DELETE FROM anthropic_model_configs WHERE id = ?', [id], (err: any) => {
                        if (err) reject(err);
                        else resolve();
                    });
                });
            ...
            case 'mongodb':
                await connection.collection('anthropic_model_configs').deleteOne({ id });
                break;

            case 'mysql':
            case 'postgresql':
                await connection.execute('DELETE FROM anthropic_model_configs WHERE id = ?', [id]);
                break;
    ...
    private parseModelConfigFromDB(row: any, dbType: string): AnthropicModelConfig {
